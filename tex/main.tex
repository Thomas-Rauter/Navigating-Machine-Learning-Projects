...
\begin{figure}[H]
    ...
    \foreach \x/\y in {5/0.65, 6/0.6, 7/0.55} {
        \draw[thick, black, ->] (\x, \y) -- (\x, \y+0.1);
    }
    \end{axis}
\end{tikzpicture}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|p{12cm}|}
        ...
        \hline
        Linear Regression & When the relationship between variables is linear and interpretability is important. Assumes no interaction unless you add them manually. \\
        \hline
        Logistic Regression & When performing binary classification and interpretability is key. Assumes feature independence unless interactions are explicitly added. \\
        \hline
        K-Nearest Neighbors (KNN) & For small to medium size datasets with local patterns, particularly in cases where model interpretability is a priority.\\
        \hline
        Naïve Bayes & When working with text classification (e.g., spam detection, sentiment analysis) or datasets where features are mostly independent. Assumes conditional independence between features. \\
        \hline
    \end{tabular}
    \caption{Supervised learning models that treat features independently (no interaction modeling).}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|p{12cm}|}
        ...
        \textbf{Model Type} & \textbf{When to Use} \\
        \hline
        Decision Tree & When you need an interpretable model that can handle non-linear relationships and structured data. Captures interactions via hierarchical splits. \\
        \hline
        Random Forest & When you need better accuracy than a single decision tree while reducing overfitting. Models feature interactions implicitly across many trees. \\
        \hline
        Support Vector Machine (SVM) & When working with small datasets that have complex, high-dimensional decision boundaries. Kernels capture non-linear feature interactions. \\
        \hline
        XGBoost & When working with structured tabular data where predictive accuracy is top priority. Models complex feature interactions automatically. \textbf{Top performer for structured data.} \\
        \hline
        CatBoost & When dealing with structured data that contains many categorical variables. Captures interactions and reduces manual preprocessing. \\
        \hline
        LightGBM & When training speed is critical, especially for large datasets. Captures feature interactions like XGBoost but optimized for speed. \\
        \hline
        Neural Networks & When dealing with unstructured data (e.g., images, text, audio) and highly complex relationships. \textbf{Neural networks are by far the best algorithm for unstructured data!} Captures deep interactions automatically. \\
        \hline
    \end{tabular}
    \caption{Supervised learning models that model interactions between features.}
\end{table}
